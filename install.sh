...
# @@File             :  install.sh
# @@Description      :  Install configurations for newsboat
...
__run_post_install() {
  ...
  if __cmd_exists newsboat; then
    if [ ! -s "$APPDIR/urls" ] || [ ! -f "$SHARE/newsboat/cache.db" ]; then
      newsboat --import-from-opml "$APPDIR/news.opml"
    fi
    if [ -f "$INSTDIR/newurls" ]; then
      if __cmd_exists rssadd; then
